function [d,si,h]=abfLoadLight(fn,varargin)
  ...
  case 3
    ...
    if verbose
      ...
      disp(['memory requirement for complete upload in matlab: '...
        num2str(round(2*h.lActualAcqLength/2^20)) ' MB']);
    end
    % recording start and stop times in seconds from midnight
    h.recTime=h.lFileStartTime;
    ...
    if doLoadData
      ...
      if length(chInd)==1 && h.nADCNumChannels>1
        ...
        [d,n]=fread(fid,h.dataPtsPerChan,[precision '=>' precision],dataSz*(h.nADCNumChannels-1));
      ...
      elseif length(chInd)/h.nADCNumChannels<1
        ...
        % --- situation (i)
        [d,n]=fread(fid,h.dataPts,[precision '=>' precision]);
